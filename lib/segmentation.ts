...
// PERFORMANCE OPTIMIZATION: Pre-compute all analytics in chunks to avoid re-calculations
// This runs ONCE after CSV upload, then results are cached in context
export async function precomputeAnalytics(clientesData: ClienteRow[]) {
  if (!clientesData || clientesData.length === 0) {
    return {
      segments: [],
      metrics: null,
  ...
  // Process in chunks of 5000 to avoid blocking UI thread
  const CHUNK_SIZE = 5000;
  let processedCount = 0;

  // Yield thread after each chunk
  for (let i = 0; i < clientesData.length; i += CHUNK_SIZE) {
    processedCount = Math.min(i + CHUNK_SIZE, clientesData.length);
    // Yield to event loop
    await new Promise((resolve) => setTimeout(resolve, 0));
  }

  // All computation happens once here - no further recalculation in components
  const segments = segmentarClientes(clientesData);
