...
import { precomputeAnalytics } from "@/lib/segmentation";
...
export function CSVUploader() {
  const { setClientesData, setVarejoData, setIsLoading, setCachedAnalytics } = useData();
    ...
    async (file: File, type: "clientes" | "varejo") => {
      ...
      try {
        ...
        if (type === "clientes") {
          ...
          // Pre-compute analytics once after upload (chunked processing)
          setIsLoading(true);
          const analytics = await precomputeAnalytics(data as ClienteRow[]);
